...
class GIFTEngine {
    ...
    evaluateMathematicalExpression(expression, fromFormat, toFormat) {
        try {
            ...
            let isConstantDefinition = false;
            
            // Check if this is a constant definition (e.g., "ξ = 6π/16")
            for (let [symbol, value] of Object.entries(constantMap)) {
                if (expression.includes(symbol + ' = ')) {
                    isConstantDefinition = true;
                    if (toFormat === 'SM') {
                        // For constant definitions, replace the symbol with its actual value
                        const actualValue = this.constants[symbol];
                        result = result.replace(new RegExp(symbol + '\\s*=\\s*[^=]+', 'g'), `${symbol} = ${actualValue.toFixed(6)}`);
                    } else if (toFormat === 'GIFT') {
                        // Keep the symbolic form for GIFT
                        result = expression;
                    }
                    containsConstants = true;
                    break;
                } else if (expression.includes(symbol)) {
                    containsConstants = true;
                    if (toFormat === 'SM') {
            ...
            // Try to evaluate mathematical expressions (but not constant definitions)
            if (!isConstantDefinition && (containsConstants || /[+\-*/^()]/.test(expression))) {
                try {
                    let evalExpr = result
                        .replace(/π/g, 'Math.PI')
                        .replace(/√/g, 'Math.sqrt')
            ...
            // Handle constant definitions specially
            if (isConstantDefinition) {
                return {
                    success: true,
                    translated: result,
                    explanation: toFormat === 'SM' ? 'Constant definition translated to numerical value' : 'Constant definition in symbolic form',
                    confidence: 0.9,
                    method: 'constant_definition',
                    calculated_value: toFormat === 'SM' ? Object.values(constantMap).find(v => result.includes(v.toFixed(6))) : null
                };
            }
