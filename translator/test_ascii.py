#!/usr/bin/env python3
"""
ASCII-only test script for the GIFT Translator (Windows-compatible)
"""
...
def test_gift_translator():
    ...
    print(f"alpha^(-1) (GIFT): {alpha_inv_gift:.6f}")
    ...
    print(f"sin^2(theta_W) (GIFT): {sin2theta_gift:.6f}")
    print(f"sin^2(theta_W) (Exp):  {sin2theta_exp:.6f}")
    ...
    print(f"xi = 5*pi/16 = {xi:.6f}")
    print(f"tau = 8*gamma^(5*pi/12) = {tau:.6f}")
    print(f"beta0 = pi/8 = {beta0:.6f}")
    print(f"delta = 2*pi/25 = {delta:.6f}")
    
    print(f"\nCorrection Factors:")
    print(f"F_alpha = 98.999")
    print(f"F_beta = 99.734")
    print(f"k = 26.464068")
